...
// Timing
long loop_timer, tiempo_motores_start, tiempo_ON, tiempo_1, tiempo_2;
...
void MPU6500_procesar() {
  ...
  if (set_gyro_angles) {
    angulo_pitch = angulo_pitch * 0.995 + angulo_pitch_acc * 0.005;
    angulo_roll  = angulo_roll  * 0.995 + angulo_roll_acc  * 0.005;
  } else {
...
void PWM() {
  ...
  while (digitalRead(pin_motor1) == HIGH || digitalRead(pin_motor2) == HIGH ||
         digitalRead(pin_motor3) == HIGH || digitalRead(pin_motor4) == HIGH) {
